...
def dataExecution(testData, outputData, outputchecker, pathdata, pathChecker):
    ...
    for index, row in mainDF.iterrows():
        ...
        mr = MTexecuter(row['output_testInput'])
         #MR1
        if row['output_MR1'] == 'd/0':
            finalDF.at[index,'MR1_checker'] ='NA -> d/0'
        else:
            finalDF.at[index,'MR1_checker'] = mr.mr_PER(row['output_MR1'])

        #MR2
        if row['output_MR2'] == 'd/0':
            finalDF.at[index,'MR2_checker'] ='NA -> d/0'
        else:
            finalDF.at[index,'MR2_checker'] = mr.mr_ADD(row['output_MR2'])

        #MR3
        if row['output_MR3'] == 'd/0':
            finalDF.at[index,'MR3_checker'] ='NA -> d/0'
        else:
            finalDF.at[index,'MR3_checker'] = mr.mr_MUL(row['output_MR3'])

        #MR4
        if row['output_MR4'] == 'd/0':
            finalDF.at[index,'MR4_checker'] ='NA -> d/0'
        else:
            finalDF.at[index,'MR4_checker'] = mr.mr_INV(row['output_MR4'])

        #MR5
        if row['output_MR5'] == 'd/0':
            finalDF.at[index,'MR5_checker'] ='NA -> d/0'
        else:
            finalDF.at[index,'MR5_checker'] = mr.mr_INC(row['output_MR5'])

        #MR6

        if type(row['output_MR6']) == str:
            # print(row['output_MR6'])
            finalDF.at[index,'MR6_checker'] ='NA -> d/0'
